...
class TemplateManager {
  ...
  // Validate item templates for data integrity
  validateItemTemplates() {
    const invalidItems = []
    
    for (const [itemId, item] of this.items) {
      // Check that only bag-type items have inventory effects
      if (item.effects?.inventory && item.type !== 'bag') {
        invalidItems.push({
          id: itemId,
          error: `Item has inventory effect but type is '${item.type}' instead of 'bag'`
        })
      }
      
      // Check that bag-type items have the correct slot
      if (item.type === 'bag' && item.slot !== 'bag') {
        invalidItems.push({
          id: itemId,
          error: `Bag-type item has slot '${item.slot}' instead of 'bag'`
        })
      }
    }
    
    if (invalidItems.length > 0) {
      console.error('\n=== ITEM TEMPLATE VALIDATION ERRORS ===')
      for (const invalid of invalidItems) {
        console.error(`❌ ${invalid.id}: ${invalid.error}`)
      }
      console.error('=== END VALIDATION ERRORS ===\n')
      throw new Error(`Found ${invalidItems.length} invalid item template(s). Please fix the above errors.`)
    }
  }
