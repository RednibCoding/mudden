...
class CombatManager {
  ...
  startCombat(player, target) {
    ...
    // Handle trade cleanup on combat start
    if (this.worldManager.commandManager) {
      this.worldManager.commandManager.handlePlayerActionCleanup(player.name, 'entered combat')
    }
