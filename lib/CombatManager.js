...
class CombatManager {
  ...
  calculatePlayerDamage(player) {
    ...
    if (player.equipment.main_hand) {
      const weapon = this.gameWorld.templateManager.getItem(player.equipment.main_hand)
  ...
  calculateEnemyDamage(enemy, player) {
    ...
    for (const slot of armorSlots) {
      if (player.equipment[slot]) {
        const armor = this.gameWorld.templateManager.getItem(player.equipment[slot])
        ...
        if (Math.random() * 100 < lootItem.chance) {
          ...
          if (playerNames.length > 0) {
            ...
            if (luckyPlayer) {
              ...
              const itemTemplate = this.gameWorld.templateManager.getItem(lootItem.id)
