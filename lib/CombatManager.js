...
class CombatManager {
  constructor(gameWorld, players, io) {
  ...
  calculatePlayerDamage(player) {
    ...
    if (player.equipment.main_hand) {
      const weapon = this.gameWorld.getItem(player.equipment.main_hand)
  ...
  calculateEnemyDamage(enemy, player) {
    ...
    for (const slot of armorSlots) {
      if (player.equipment[slot]) {
        const armor = this.gameWorld.getItem(player.equipment[slot])
  ...
  calculateEnemyRewards(enemy, playerNames) {
    ...
    if (playerNames.length > 0) {
      rewardMessage += `Each player gains ${baseExp} experience and ${baseGold} gold.`
    }
        ...
        if (Math.random() * 100 < lootItem.chance) {
          ...
          if (playerNames.length > 0) {
            ...
            if (luckyPlayer) {
              ...
              const itemTemplate = this.gameWorld.getItem(lootItem.id)
  ...
  removePlayerFromCombat(playerName) {
    const sessionId = this.playerToSession.get(playerName)
  ...
  updateQuestProgress(player, type, target, quantity) {
    if (this.gameWorld.questSystem) {
      return this.gameWorld.questSystem.updateQuestProgress(player, type, target, quantity)
    }
    return false
  }
}
