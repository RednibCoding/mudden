...
class Player {
  constructor(name) {
    ...
    // Health recovery system (tick-based)
    this.healthRecoveryCallback = null
  ...
  getTotalStats(gameWorld = null) {
    ...
    if (this.equipment && gameWorld) {
      for (const [slot, itemId] of Object.entries(this.equipment)) {
        if (itemId) {
          const itemTemplate = gameWorld.templateManager.getItem(itemId)
  ...
  startHealthRecovery(updateCallback) {
    ...
    // Health recovery is now handled by the global tick system
    // Just mark player as eligible for recovery
    this.healthRecoveryCallback = updateCallback
  }
  ...
  processHealthRecoveryTick() {
    if (this.inCombat || this.health >= this.maxHealth || !this.healthRecoveryCallback) {
    ...
    if (this.health >= this.maxHealth && oldHealth < this.maxHealth) {
      this.healthRecoveryCallback(this, "You feel completely refreshed and at full health.")
      this.stopHealthRecovery()
    } else {
      // Send silent update for health bar
      this.healthRecoveryCallback(this, null)
    }
