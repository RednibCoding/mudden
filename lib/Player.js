...
class Player {
  ...
  addExperience(amount) {
    ...
    while (totalExpNeeded <= this.experience) {
      const expForNextLevel = 100 + (newLevel - 1) * 50 // Increases by 50 each level
      totalExpNeeded += expForNextLevel
      if (totalExpNeeded <= this.experience) {
        newLevel++
      }
    }
  ...
  syncQuestProgressWithInventory() {
    ...
    this.quests.forEach(playerQuest => {
      if (playerQuest.status !== 'accepted' || !playerQuest.objectives) return
