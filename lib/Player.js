...
class Player {
  constructor(name, initialArea = null, initialRoom = null) {
    this.name = name
  ...
  getTotalStats(worldManager = null) {
    const totalStats = { ...this.stats }
    
    // Add equipment bonuses if WorldManager reference is provided
    if (this.equipment && worldManager) {
      for (const [slot, itemId] of Object.entries(this.equipment)) {
        if (itemId) {
          const itemTemplate = worldManager.templateManager.getItem(itemId)
