...
class InventoryCommands extends BaseCommand {
  ...
  inventory(player) {
    ...
    const totalCapacity = this.worldManager.inventoryManager.getInventoryCapacity(player)
    
    if (displayItems.length === 0) {
      return `Your inventory is empty. (${freeSlots}/${totalCapacity} slots free)`
    }
    
    let result = `\n=== Inventory (${displayItems.length}/${totalCapacity}) ===\n`
    ...
    if (freeSlots > 0) {
      result += `\n${freeSlots} empty slots remaining.`
    }
  ...
  use(player, args) {
    ...
    const enrichedInventory = player.inventory
      .filter(invItem => invItem !== null) // Filter out null slots
      .map(invItem => {
  ...
  equip(player, args) {
    ...
    const result = this.worldManager.equipmentManager.equipItemByName(player, targetName)
  ...
  unequip(player, args) {
    ...
    const targetName = args.join(' ')
    const result = this.worldManager.equipmentManager.unequipItemByName(player, targetName)
  ...
  drop(player, args) {
    ...
    const inventoryItem = this.worldManager.inventoryManager.findItemInInventory(player, itemQuery)
    if (!inventoryItem) {
      const suggestions = this.worldManager.inventoryManager.getSimilarItems(player.inventory, itemQuery)
    ...
    if (quantity > inventoryItem.quantity) {
      return `You only have ${inventoryItem.quantity} ${this.worldManager.inventoryManager.getItemName(inventoryItem.id)}, but tried to drop ${quantity}.`
    }
    ...
    const itemName = this.worldManager.inventoryManager.getItemName(inventoryItem.id)
