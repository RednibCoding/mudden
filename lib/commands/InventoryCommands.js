  ...
  take(player, args) {
    ...
    for (const itemName of itemNames) {
      ...
      if (item._respawnConfig?.onetime && !item._respawnConfig?.quest) {
        player.markOnetimeItemTaken(player.currentArea, player.currentRoom, item.id)
      }
    ...
    if (item.type === 'consumable' || item.type === 'scroll') {
      if (item.effects) {
          ...
          if (!combat) {
            ...
            if (combat.defender.data.health <= 0) {
              ...
              if (combatCommands) {
                ...
                // Clear combat manually if we can't access CombatCommands
        ...
        if (item.effects.spell) {
          result += ` The scroll glows with ${item.effects.spell} magic!`
        }
