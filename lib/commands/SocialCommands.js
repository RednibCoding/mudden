...
class SocialCommands extends BaseCommand {
  ...
  say(player, args) {
    ...
    const channel = player.channel || 'room'
    let targetPlayers = []
    let channelTag = ''
    
    // Determine target players based on channel
    switch (channel) {
      case 'room':
        targetPlayers = this.getPlayersInRoom(player.currentArea, player.currentRoom, player)
        channelTag = ''
        break
      case 'area':
        targetPlayers = this.getPlayersInArea(player.currentArea, player)
        channelTag = ' <area>'
        break
      case 'world':
        targetPlayers = this.getAllPlayers(player)
        channelTag = ' <world>'
        break
      default:
        return "Invalid channel. Use 'channel room', 'channel area', or 'channel world'."
    }
    
    // Send message to target players
    targetPlayers.forEach(otherPlayer => {
      const socket = global.getPlayerSocket(otherPlayer.name)
      if (socket) {
        socket.emit('output', `[SAY] ${player.name}${channelTag}: "${message}"`)
      }
    })
    
    return `[SAY] You${channelTag}: "${message}"`
  }

  channel(player, args) {
    if (args.length === 0) {
      return `Current channel: ${player.channel || 'room'}\nAvailable channels: room, area, world`
    }
    
    const newChannel = args[0].toLowerCase()
    
    if (!['room', 'area', 'world'].includes(newChannel)) {
      return "Invalid channel. Available channels: room, area, world"
    }
    
    player.channel = newChannel
    // Note: Channel preference is session-only, not saved to file
    
    return `Channel set to: ${newChannel}`
  }
  ...
  tell(player, args) {
    ...
    if (!targetPlayer) {
      ...
      const formattedName = targetName.charAt(0).toUpperCase() + targetName.slice(1).toLowerCase()
      return `Player ${formattedName} is not online.`
    }
    ...
    // Send private message using proper socket communication
    const socket = this.getPlayerSocket(targetPlayer.name)
    if (socket) {
      socket.emit('output', `[TELL] ${player.name} tells you: "${message}"`)
      
      // Store last tell sender for reply functionality
      targetPlayer.lastTellSender = player.name
    }
    
    return `[TELL] You tell ${targetPlayer.name}: "${message}"`
  }

  reply(player, args) {
    if (args.length === 0) {
      return "Usage: reply <message> or r <message>"
    }

    // Check if player has received any tells
    if (!player.lastTellSender) {
      return "You haven't received any private messages yet."
    }

    const targetName = player.lastTellSender
    const message = args.join(' ')
    
    // Check if the original sender is still online
    const targetPlayer = this.findPlayerByName(targetName)
    if (!targetPlayer) {
      const formattedName = targetName.charAt(0).toUpperCase() + targetName.slice(1).toLowerCase()
      return `Player ${formattedName} is not online.`
    ...
    // Use the same logic as tell command
    const socket = this.getPlayerSocket(targetPlayer.name)
    if (socket) {
      socket.emit('output', `[TELL] ${player.name} tells you: "${message}"`)
  ...
  talk(player, args) {
    ...
    if (npc.quests && npc.quests.length > 0) {
      const questSystem = this.worldManager.questSystem
      if (questSystem) {
        const availableQuests = npc.quests
          .map(qId => this.worldManager.templateManager.getQuest(qId))
  ...
  ask(player, args) {
    if (args.length < 3) {
      return "Usage: ask <npc> about <topic> (or ask <npc> a <topic>)"
    }

    // Find the "about" or "a" keyword in the arguments
    let aboutIndex = -1
    for (let i = 1; i < args.length; i++) {
      const word = args[i].toLowerCase()
      if (word === 'about' || word === 'a') {
        aboutIndex = i
        break
      }
    }

    if (aboutIndex === -1 || aboutIndex >= args.length - 1) {
      return "Usage: ask <npc> about <topic> (or ask <npc> a <topic>)"
    }

    const npcName = args.slice(0, aboutIndex).join(' ')
    const topic = args.slice(aboutIndex + 1).join(' ').toLowerCase()
    const room = this.getCurrentRoom(player)
