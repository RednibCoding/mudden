...
class QuestCommands extends BaseCommand {
  ...
  quest(player, args) {
    ...
    switch (action) {
      ...
      default:
        return "Quest actions: list, info, accept, abandon, complete, look"
    }
  }
  ...
  questLook(player, itemName) {
    ...
    // Get all reward items from active quests
    const activeQuests = this.getActiveQuests(player)
    const rewardItems = []
    ...
    for (const quest of activeQuests) {
      if (quest.rewards && quest.rewards.items) {
        for (const itemId of quest.rewards.items) {
          const itemTemplate = this.gameWorld.templateManager.getItem(itemId)
          if (itemTemplate) {
            rewardItems.push({
              item: itemTemplate,
    ...
    let result = `\n=== ${item.name} (Quest Reward from "${bestMatch.questName}") ===\n`
  ...
  questInfo(player, questName) {
    ...
    result += `\nQuest Giver: ${this.gameWorld.templateManager.getNPC(quest.giver)?.name || quest.giver}\n`
    ...
    if (quest.rewards.experience || quest.rewards.gold || quest.rewards.items?.length) {
      ...
      if (quest.rewards.items?.length) {
        quest.rewards.items.forEach(itemId => {
          const item = this.gameWorld.templateManager.getItem(itemId)
  ...
  acceptQuest(player, questName) {
    ...
    if (!questGiver) {
      const giverNpc = this.gameWorld.templateManager.getNPC(quest.giver)
    ...
    for (const objective of quest.objectives) {
      if (objective.type === OBJECTIVE_TYPES.COLLECT && objective.givenByQuestGiver) {
        ...
        const itemTemplate = this.gameWorld.templateManager.getItem(objective.target)
  ...
  getActiveQuests(player) {
    ...
    return player.activeQuests
      .map(pq => this.gameWorld.templateManager.getQuest(pq.id))
      .filter(Boolean)
  }
  ...
  getCompletedQuests(player) {
    ...
    return player.completedQuests
      .map(questId => this.gameWorld.templateManager.getQuest(questId))
      .filter(Boolean)
  }
  ...
  getQuestProgress(player, questId) {
    ...
    const quest = this.gameWorld.templateManager.getQuest(questId)
    if (!quest) return "0%"
  ...
  updateQuestProgress(player, type, targetId, amount = 1) {
    ...
    for (const playerQuest of player.activeQuests) {
      ...
      const quest = this.gameWorld.templateManager.getQuest(playerQuest.id)
      if (!quest) continue
  ...
  completeQuest(player, questName) {
    ...
    if (!turnInNpc) {
      const npcTemplate = this.gameWorld.templateManager.getNPC(turnInNpcId)
  ...
  turnInQuest(player, questId) {
    ...
    if (quest.rewards.experience) {
      ...
      if (levelsGained > 0) {
        rewardMessage += `(LEVEL UP! Now level ${player.level}) `
      }
    }
