...
class QuestCommands extends BaseCommand {
  ...
  acceptQuest(player, questName) {
    ...
    // Handle delivery quests - give required items to player
    let deliveryMessage = ""
    if (quest.type === 'delivery') {
      for (const objective of quest.objectives) {
        if (objective.type === 'deliver' && objective.item) {
          // Add the delivery item to player's inventory
          player.addItem(objective.item, objective.quantity)
          
          
          const itemTemplate = this.gameWorld.getItem(objective.item)
          const itemName = itemTemplate ? itemTemplate.name : objective.item
  ...
  abandonQuest(player, questName) {
    ...
    // Handle delivery quest item removal
    let removedItemsMessage = ""
    if (quest.type === 'delivery') {
      for (const objective of quest.objectives) {
        if (objective.type === 'deliver' && objective.item) {
            // Find and remove delivery items from inventory
            ...
            if (playerItem) {
              ...
              player.removeItem(objective.item, removeQuantity)
  ...
  hasActiveQuest(player, questId) {
    ...
    return playerQuest && playerQuest.status === 'accepted'
  }

  hasCompletedQuest(player, questId) {
    const playerQuest = this.getPlayerQuest(player, questId)
    return playerQuest && playerQuest.status === 'completed'
  }

  getActiveQuests(player) {
    if (!player.quests) return []
    return player.quests
      .filter(pq => pq.status === 'accepted')
  ...
  updateQuestProgress(player, type, target, quantity = 1) {
    ...
    for (const playerQuest of player.quests) {
      if (playerQuest.status !== 'accepted') continue
      ...
      if (isComplete && playerQuest.status === 'accepted') {
  ...
  completeQuest(player, questName) {
    ...
    const playerQuest = this.getPlayerQuest(player, quest.id)
    if (!playerQuest || playerQuest.status !== 'accepted') {
      return `Quest "${quest.name}" is not active.`
    }
    ...
    if (!isComplete) {
      return `Quest "${quest.name}" objectives are not completed yet.`
    }

    // Use the turnInQuest method for the actual completion logic
  ...
  turnInQuest(player, questId) {
    ...
    // Remove items from inventory for all collect/deliver objectives
    for (const objective of quest.objectives) {
      if (objective.type === 'collect' || objective.type === 'deliver') {
        const itemId = objective.target || objective.item
        console.log(`DEBUG: Removing ${objective.quantity}x ${itemId} from inventory`)
        console.log(`DEBUG: Player has item before removal:`, player.hasItem(itemId, objective.quantity))
        const removed = player.removeItem(itemId, objective.quantity)
        console.log(`DEBUG: Item removal result:`, removed)
        console.log(`DEBUG: Player has item after removal:`, player.hasItem(itemId, objective.quantity))
      }
    }
