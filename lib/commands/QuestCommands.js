...
class QuestCommands extends BaseCommand {
  ...
  quest(player, args) {
    if (args.length === 0) {
      return "Usage: quest <info|accept|abandon> [quest_name]"
    }
    ...
    switch (action) {
      ...
      default:
        return "Quest actions: info, accept, abandon"
    }
  }
  ...
  acceptQuest(player, questName) {
    ...
    return `Quest accepted: ${quest.name}\nCheck your quest journal with 'quests' command.${deliveryMessage}`
  }
  ...
  turnInQuest(player, questId) {
    ...
    // Check if all objectives are complete (special handling for delivery quests)
    let isComplete = false
    let deliveryItemsToRemove = []

    if (quest.type === 'delivery') {
      isComplete = quest.objectives.every((objective, index) => {
        if (objective.type === 'deliver') {
          // Check if player has the delivery item
          const playerItem = player.inventory.find(item => item.id === objective.item)
          if (playerItem && playerItem.quantity >= objective.quantity) {
            deliveryItemsToRemove.push({ id: objective.item, quantity: objective.quantity })
            return true
          }
          return false
        }
    ...
    for (const item of deliveryItemsToRemove) {
      player.removeItem(item.id, item.quantity)
    }

    // Remove collection items from inventory for collection quests
    if (quest.type !== 'delivery') {
      for (const objective of quest.objectives) {
