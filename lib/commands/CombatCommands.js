...
class CombatCommands extends BaseCommand {
    ...
    if (combat.defender.type === 'enemy') {
      ...
      if (enemy.loot) {
        ...
        if (lootResult.items.length > 0) {
          ...
          // Add items to inventory and display grouped loot
          Object.entries(itemCounts).forEach(([itemId, quantity]) => {
            attacker.addItem(itemId, quantity)
            const item = this.gameWorld.getItem(itemId)
            const itemName = item ? item.name : itemId
            if (quantity === 1) {
              result += `You loot: ${itemName}\n`
            } else {
              result += `You loot: ${itemName} (${quantity})\n`
            }
          })
        }
      }
  ...
  combatStatus(player, args) {
    ...
    result += `${attacker.name}: ${attacker.health}/${attacker.maxHealth}\n`
    result += `${defender.name}: ${defender.health}/${defender.maxHealth || defender.stats?.maxHealth || defender.stats?.health}`
