...
class CombatCommands extends BaseCommand {
  ...
  processCombatSession(sessionId, session) {
    ...
    for (const enemy of aliveEnemies) {
      ...
      // Enemy chooses random player target
      const targetPlayerName = session.players[Math.floor(Math.random() * session.players.length)]
      ...
      if (targetPlayer) {
        ...
        if (targetPlayer.health <= 0) {
          const defeatMessage = this.handlePlayerDefeat(targetPlayer)
          combatMessages.push(defeatMessage)
          
          // Remove defeated player from session
