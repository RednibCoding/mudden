...
class CombatCommands extends BaseCommand {
  ...
  getValidTargets(room, player) {
    ...
    if (room.npcs && room.npcs.length > 0) {
      const attackableNpcs = room.npcs.filter(npc => {
        const npcData = this.gameWorld.templateManager.getNPC(npc.id)
  ...
  homestone(player, args) {
    ...
    const npcData = this.gameWorld.templateManager.getNPC(homestoneNpc.id)
  ...
  // Cleanup unused enemy instances (delegate to GameWorld)
  cleanupUnusedEnemyInstance(areaId, roomId, enemyId) {
    return this.gameWorld.removeSharedEnemyInstance(areaId, roomId, enemyId)
  }
}
