...
class InfoCommands extends BaseCommand {
  ...
  stats(player) {
    ...
    // Show equipped items using EquipmentManager
    const equipmentDisplay = this.worldManager.equipmentManager.getEquipmentDisplay(player)
    if (equipmentDisplay !== "You have no equipment.") {
      result += `\n${equipmentDisplay}`
    }
  ...
  equipment(player) {
    ...
    // Get equipment display from EquipmentManager
    const equipmentDisplay = this.worldManager.equipmentManager.getEquipmentDisplay(player)
    result += equipmentDisplay
    
    // Show equipment stats
    const equipmentStats = this.worldManager.equipmentManager.getEquipmentStats(player)
    if (equipmentStats.strength > 0 || equipmentStats.defense > 0 || equipmentStats.speed > 0) {
      result += `\n\nEquipment Bonuses:`
      if (equipmentStats.strength > 0) result += `\n  Strength: +${equipmentStats.strength}`
      if (equipmentStats.defense > 0) result += `\n  Defense: +${equipmentStats.defense}`
      if (equipmentStats.speed > 0) result += `\n  Speed: +${equipmentStats.speed}`
    }
  ...
  lookAtInventoryItem(player, target) {
    ...
    for (const inventoryItem of player.inventory) {
      // Skip null slots in slot-based inventory
...
INVENTORY:
  ...
  drop [item] [quantity]  - Drop/destroy an item from inventory
  use [item]              - Use/consume an item
  equip, wear [item]      - Equip a weapon or armor
  unequip, remove [item]  - Unequip an item
  equipment, eq           - Show detailed equipment information
  consolidate             - Combine stackable items to free up inventory slots
...
TRADING:
  ...
  trade offer <item> [qty] - Add item to your trade offer (only tradeable items)
