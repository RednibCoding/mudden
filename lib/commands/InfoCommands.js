...
class InfoCommands extends BaseCommand {
    ...
    if (room) {
      ...
      if (room.enemies) {
        ...
        if (enemy) {
          ...
          if (enemy.stats) {
            if (enemy.stats.health) {
              result += `Health: ${enemy.stats.health}/${enemy.stats.maxHealth || enemy.stats.health}\n`
            }
  ...
  lookAtRoom(player) {
    ...
    console.log(`[DEBUG] Looking for players in ${player.currentArea}.${player.currentRoom}, found: ${otherPlayers.length}`)
