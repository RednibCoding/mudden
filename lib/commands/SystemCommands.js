...
class SystemCommands extends BaseCommand {
  ...
  templates(player, args) {
    if (!args || args.length === 0) {
      ...
      const counts = this.gameWorld.templateManager.getTemplateCounts()
...
Usage: templates <reload|item|enemy|npc|quest> [id]`
    ...
    switch (command) {
      case 'reload':
        try {
          this.gameWorld.templateManager.reloadAllTemplates()
      ...
      case 'item':
        ...
        const enrichedItem = this.gameWorld.templateManager.getEnrichedItem(id)
...
Has Stats: ${enrichedItem.hasStats}`
      ...
      case 'enemy':
        ...
        const enrichedEnemy = this.gameWorld.templateManager.getEnrichedEnemy(id)
...
Accuracy: ${enrichedEnemy.accuracy}%`
      ...
      case 'npc':
        ...
        const enrichedNPC = this.gameWorld.templateManager.getEnrichedNPC(id)
...
Can Trade: ${enrichedNPC.canTrade}`
      ...
      case 'quest':
        ...
        const enrichedQuest = this.gameWorld.templateManager.getEnrichedQuest(id)
